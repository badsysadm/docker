...
if [ "$1" = "unitd" ] || [ "$1" = "unitd-debug" ]; then
    if /usr/bin/find "/var/lib/$APPNAME/" -mindepth 1 -print -quit 2>/dev/null | /bin/grep -q .; then
        ...
        if /usr/bin/find "/var/lib/$APPNAME/docker-entrypoint.d/" -mindepth 1 -print -quit 2>/dev/null | /bin/grep -q .; then
            ...
            echo "$0: Looking for configuration snippets in /var/lib/$APPNAME/docker-entrypoint.d/..."
            for f in $(/usr/bin/find /var/lib/$APPNAME/docker-entrypoint.d/ -type f -name "*.json"); do
                echo "$0: Applying configuration $f";
                curl_put $f "config"
            done

            echo "$0: Looking for shell scripts in /var/lib/$APPNAME/docker-entrypoint.d/..."
            for f in $(/usr/bin/find /var/lib/$APPNAME/docker-entrypoint.d/ -type f -name "*.sh"); do
                echo "$0: Launching $f";
                "$f"
            done

            # warn on filetypes we don't know what to do with
            for f in $(/usr/bin/find /var/lib/$APPNAME/docker-entrypoint.d/ -type f -not -name "*.sh" -not -name "*.json" -not -name "*.pem"); do
                echo "$0: Ignoring $f";
            done
            ...
            echo "$0: /var/lib/$APPNAME/docker-entrypoint.d/ is empty, skipping initial configuration..."
        fi
    fi
fi
